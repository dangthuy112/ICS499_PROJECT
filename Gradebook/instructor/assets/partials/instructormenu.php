...
while ($row = mysqli_fetch_array($result)) {
    if ($row['semester'] == 'Current Semester') {
        ...
        echo "<a  href='./instructor-course.php?coursenumber=$row[coursenumber]&course=$row[subject]&courseid=$row[courseID]&iid=$stringiid&option=0'>$row[subject]$row[coursenumber]</a><br>";
    }
}
echo
" </div>
